    ...
    <div className="min-h-screen w-full flex flex-col font-[Inter] bg-gray-50">
      <div className="container mx-auto px-4 py-6 md:p-8 max-w-7xl flex-grow">
        <div className="relative flex justify-center items-center pb-6 mb-8">
          <Link to="/" className="absolute left-0 top-1/2 -translate-y-1/2 flex items-center gap-2 bg-white text-gray-700 hover:bg-[#57B952] hover:text-white px-4 py-2 rounded-lg shadow border border-gray-300 hover:border-[#57B952] transition-all">
        ...
        <div className="bg-white rounded-xl shadow border border-gray-200 p-4 md:p-5 mb-6 flex items-center gap-2 flex-wrap">
          {[
            { key: 'all', label: 'Todos' },
        ...
        {loading ? (
          ...
          <div className="text-center text-gray-500 py-20 bg-white rounded-xl border border-gray-200">Nenhum favorito encontrado.</div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map(fav => (
              <div key={fav.id} className="bg-white rounded-xl shadow border border-gray-200 p-5 relative">
                ...
                <h3 className="text-lg font-bold text-gray-900 mb-1" title={fav.name || fav.titulo}>{fav.name || fav.titulo}</h3>
                {fav.descricao && (
                ...
                <div className="flex gap-2">
                  ...
                  {fav.url && fav.type === 'tutorial' && (
                    <a href={fav.url} download className="flex-1 flex items-center justify-center px-3 py-2 rounded-lg text-sm font-semibold text-gray-700 bg-gray-100 hover:bg-gray-200 border border-gray-300">
                      <Download size={16} className="mr-2" /> Baixar
                    </a>
                  )}
                  <button onClick={() => handleRemove(fav)} className="px-3 py-2 rounded-lg text-sm font-semibold text-gray-700 bg-white hover:bg-gray-100 border border-gray-300">Remover</button>
                </div>
              </div>
            ))}
          </div>
