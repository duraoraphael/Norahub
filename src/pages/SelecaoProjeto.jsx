    ...
    {/* ThemeToggle removed */}
      ...
      <main className="flex-grow flex flex-col items-center p-8">
        <div className="w-full max-w-6xl">
            <div className="flex flex-col md:flex-row justify-between items-end mb-8 gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-gray-900">Seleção de Base</h1>
                ...
                <div className="flex gap-3">
                    ...
                    {isAdmin && (
                        <Link 
                            to="/admin" 
                            className="bg-purple-100 bg-purple-900/30 text-purple-700 text-purple-300 px-4 py-2 rounded-lg font-bold flex items-center gap-2 shadow transition-transform hover:scale-105 text-sm border border-purple-200 border-purple-800"
                        >
            ...
            {loading ? (
                ...
                <div className="text-center py-20 bg-white bg-gray-800 rounded-xl shadow border border-gray-200 border-gray-700">
                    <p className="text-gray-500 text-gray-400 mb-4">Nenhuma base cadastrada ainda.</p>
                    {isAdmin && (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {projetos.map((projeto) => (
                        <div 
                            ...
                            className="group bg-white bg-gray-800 p-8 rounded-xl shadow-md hover:shadow-xl border border-gray-200 border-gray-700 text-left transition-all hover:-translate-y-1 flex flex-col h-full relative cursor-pointer"
                        >
                            {isAdmin && (
                                <button 
                            ...
                            <div className="flex items-start justify-between mb-4">
                                <div className="bg-green-100 bg-green-900/30 p-3 rounded-lg text-[#57B952]"><Briefcase size={24} /></div>
                            ...
                            <div className="mt-auto w-full py-2 rounded-lg bg-gray-50 bg-green-600 text-center text-sm font-medium text-white text-gray-400 group-hover:bg-[#57B952] group-hover:text-white transition-colors">Acessar Projeto</div>
                        </div>
                    ))}
                </div>
            )}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden border border-gray-200 border-gray-700 animate-fade-in">
                <div className="flex justify-between items-center p-6 border-b border-gray-100 border-gray-700">
                    <h2 className="text-xl font-bold text-gray-900 text-white">Adicionar Nova Base</h2>
                ...
                <form onSubmit={handleSaveProject} className="p-6 space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-gray-700 text-gray-300 mb-1">Nome do Projeto</label>
                        <input type="text" placeholder="Ex: Projeto 743 - Facilities" value={newProjectName} onChange={(e) => setNewProjectName(e.target.value)} className="w-full px-4 py-2 rounded-lg bg-gray-50 bg-gray-700 border border-gray-300 border-gray-600 text-gray-900 text-white focus:ring-2 focus:ring-[#57B952] outline-none" required />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 text-gray-300 mb-1 flex items-center gap-2"><FileText size={14} /> Link do Forms (Solicitação)</label>
                        <input type="url" placeholder="https://forms..." value={urlForms} onChange={(e) => setUrlForms(e.target.value)} className="w-full px-4 py-2 rounded-lg bg-gray-50 bg-gray-700 border border-gray-300 border-gray-600 text-gray-900 text-white focus:ring-2 focus:ring-[#57B952] outline-none" required />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 text-gray-300 mb-1 flex items-center gap-2"><Share size={14} /> Link do SharePoint (Aprovação)</label>
                        <input type="url" placeholder="https://sharepoint..." value={urlSharePoint} onChange={(e) => setUrlSharePoint(e.target.value)} className="w-full px-4 py-2 rounded-lg bg-gray-50 bg-gray-700 border border-gray-300 border-gray-600 text-gray-900 text-white focus:ring-2 focus:ring-[#57B952] outline-none" required />
                    </div>
                    <div className="pt-4 flex gap-3">
                        <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 py-2 rounded-lg text-gray-600 text-gray-300 hover:bg-gray-100 hover:bg-gray-700 font-medium transition-colors">Cancelar</button>
      ...
      <footer className="w-full py-6 text-center text-gray-500 text-xs shrink-0 border-t border-gray-200 border-gray-800 bg-white bg-white">&copy; 2025 Parceria Petrobras & Normatel Engenharia</footer>
    </div>
  );
}
